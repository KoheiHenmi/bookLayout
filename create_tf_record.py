"""
Usage:
    python create_tf_record.py --annotations_dir=`pwd`/annotations --images_dir=`pwd`/images --output_dir=`pwd`/data/ --label_map_path=`pwd`/data/manhole_label_map.pbtxt
"""
...
import sys
from util import read_json_list,recursive_parse_example_to_dict
...
def create_tf_record(output_path,
    ...
    for idx, example in enumerate(examples):
        ...
        example_dict = recursive_parse_example_to_dict(example)
...
def main(_):
    ...
    train_output_path = os.path.join(FLAGS.output_dir, 'bookLayout_train.record')
    val_output_path = os.path.join(FLAGS.output_dir, 'bookLayout_val.record')
    ...
    examples = read_json_list(annotations_dir,images_dir)
    
    examples  # JPG画像のパスを追加
    random.seed(42)
    random.shuffle(examples)
    num_examples = len(examples)

    num_train = int(1.0 * num_examples)
